...
records. CorpusSLR compiles one structured query into the syntax of each
supported database, retrieves through native APIs or parses file exports,
deduplicates with an auditable cascade, and generates the PRISMA 2020 flow
diagram and the PRISMA-S search appendix from recorded provenance rather than
reconstructing them afterwards. Deduplication reaches F1 0.9996 on the
published ASySD gold standard and a median pairwise F1 of 0.9592 across fifteen
disciplines under an identifier-blind protocol. The corpus is written in Scopus
export layout, verified by running \texttt{bibliometrix::convert2df()} on it.
\end{abstract}
...
%% ------------------------------------------------------------------
%% Code metadata table (SoftwareX requirement, fields C1-C8).
%% Template version 6 (March 2026) carries eight rows and no "Reproducible
%% Capsule" row; an earlier version had one as C3, which shifted every label
%% below it by one. The browser-runnable notebook that row used to hold is
%% named in the illustrative example instead.
%% ------------------------------------------------------------------
\begin{table*}[t]
...
\url{https://s-matysik.github.io/CorpusSLR/}, built from
\texttt{docs/user\_guide.md} and the generated
\texttt{docs/api\_reference.md} \\
...
The same workflow runs in the browser with no local installation, as a
notebook of 26 cells:
\url{https://colab.research.google.com/github/s-matysik/CorpusSLR/blob/main/notebooks/corpusslr_colab.ipynb}.
...
with no false positives and one missed duplicate against ASySD's two. Reaching
that was not threshold tuning. Rejecting the literal \texttt{NA} as an
identifier removed 492 spurious links in this file alone, moving the
measurement from one false positive and nine misses to none and four
(\texttt{validation/na\_doi\_fix\_impact.csv}); the separators described above
account for the rest. Two of the five error classes fixed to reach this level
were not threshold problems but normalisation defects that no amount of tuning
would have removed: percent-encoded DOIs creating a second identity for the
same work, and page fields carrying an article length rather than a location.
